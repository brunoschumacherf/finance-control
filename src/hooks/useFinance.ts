...
export const useFinance = (): FinanceState => {
  ...
  useEffect(() => {
    ...
    const unsubscribeExpenses = onSnapshot(query(collection(db, 'expenses'), orderBy('createdAt', 'desc')), (snapshot) => {
      setExpenses(snapshot.docs.map((item) => ({ id: item.id, ...(item.data() as Omit<Expense, 'id' | 'createdAt'>), createdAt: toDate(item.data().createdAt) })));
      setLoading(false);
    });
    const unsubscribeIncomes = onSnapshot(query(collection(db, 'incomes'), orderBy('createdAt', 'desc')), (snapshot) => {
      setIncomes(snapshot.docs.map((item) => ({ id: item.id, ...(item.data() as Omit<Income, 'id' | 'createdAt'>), createdAt: toDate(item.data().createdAt) })));
    });
  ...
  const limiteMaisUsado = useMemo<Limit | undefined>(() => [...limits].sort((a, b) => ((b.limite - b.restante) / b.limite) - ((a.limite - a.restante) / a.limite))[0], [limits]);

  return { config, expenses, incomes, limits, loading, totalGasto, totalRecebido, limiteMaisUsado };
};
